...
public class ProjectMeasuresIndex extends BaseIndex {
  ...
  private static void addFacets(SearchRequestBuilder esSearch, SearchOptions options, Map<String, QueryBuilder> filters) {
    ...
    if (options.getFacets().contains(FILTER_LANGUAGES)) {
      esSearch.addAggregation(createStickyFacet(FILTER_LANGUAGES, filters, createLanguagesFacet()));
    }
  }
  ...
  private static AbstractAggregationBuilder createLanguagesFacet() {
    return AggregationBuilders.nested("nested_" + FILTER_LANGUAGES)
      .path(FIELD_LANGUAGES)
      .subAggregation(
        AggregationBuilders.terms(FILTER_LANGUAGES)
          .field(FIELD_LANGUAGES_KEY)
          .subAggregation(AggregationBuilders.sum("size_" + FILTER_LANGUAGES)
  ...
  private Map<String, QueryBuilder> createFilters(ProjectMeasuresQuery query) {
    ...
    query.getLanguages()
      .ifPresent(languages -> filters.put(FILTER_LANGUAGES,
