...
public class SettingsWsSupport {
  ...
  boolean isVisible(String key, @Nullable PropertyDefinition definition, Optional<ComponentDto> component) {
    return hasPermission(GlobalPermission.SCAN, SCAN_EXECUTION, component) || (verifySecuredSetting(key, definition, component) && (verifyLicenseSetting(key, definition)));
  }

  private boolean verifySecuredSetting(String key, @Nullable PropertyDefinition definition, Optional<ComponentDto> component) {
    return isLicense(key, definition) || (!key.endsWith(DOT_SECURED) || hasPermission(GlobalPermission.ADMINISTER, ADMIN, component));
  }
  ...
  private boolean hasPermission(GlobalPermission globalPermission, String projectPermission, Optional<ComponentDto> component) {
    if (userSession.hasPermission(globalPermission, defaultOrganizationProvider.get().getUuid())) {
      return true;
    }
    return component
