...
public class ServerUserSession extends AbstractUserSession {
  ...
  private Map<String, EnumSet<GlobalPermission>> permissionsByOrganizationUuid;
  ...
  protected boolean hasPermissionImpl(GlobalPermission permission, String organizationUuid) {
    ...
    Set<GlobalPermission> permissions = permissionsByOrganizationUuid.computeIfAbsent(organizationUuid, this::loadOrganizationPermissions);
    return permissions.contains(permission);
  }

  private EnumSet<GlobalPermission> loadOrganizationPermissions(String organizationUuid) {
    ...
    return permissionKeys.stream()
      ...
      .collect(Collectors.toCollection(() -> EnumSet.noneOf(GlobalPermission.class)));
  }
  ...
  private boolean loadIsSystemAdministrator() {
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      if (!organizationFlags.isEnabled(dbSession)) {
        ...
        return hasPermission(GlobalPermission.ADMINISTER, uuidOfDefaultOrg);
      }
